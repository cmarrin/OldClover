...
- [x] Get rid of Op::Else and OpEndIf. Have Op::If jump to else clause and at the end of if clause do an Op::Jump past else clause
- [x] Clean up for loop to not have so many jumps. Save iterator code to the side and insert it at the end of the for loop
...
- [ ] Make Op::LAnd and Op::LOr skip the right expression when the left expression is false (Op::LAnd) or true (Op::LOr). So we can get rid of these ops
