...
namespace fmt {
    ...
    public:
        ...
        static uint32_t toString(char* buf, uint16_t size, uint32_t v, uint8_t base = 10, Capital cap = Capital::No)
        {
            Generator g(buf, size);
            return printString(g, v, base, cap);
        }
        
        static uint32_t toString(char* buf, uint16_t size, int64_t v)
        {
            Generator g(buf, size);
            return printString(g, v);
        }
        
        static uint32_t toString(char* buf, uint16_t size, uint64_t v, uint8_t base = 10, Capital cap = Capital::No)
        {
            Generator g(buf, size);
            return printString(g, v, base, cap);
        }
        ...
        {
            return toString(buf, size, static_cast<int32_t>(v));
        }
        
        static uint32_t toString(char* buf, uint16_t size, uint8_t v, uint8_t base = 10, Capital cap = Capital::No)
        {
            return toString(buf, size, static_cast<uint32_t>(v), base, cap);
        }
        ...
        static uint32_t toString(char* buf, uint16_t size, uint16_t v, uint8_t base = 10, Capital cap = Capital::No)
        {
            return toString(buf, size, static_cast<uint32_t>(v), base, cap);
        }
